...
TcpServer server(5577,
{120, 1, 1}, // Keep alive{idle:1s, interval: 1s, pk_count: 1}
...
[](DataBuffer data, TcpServer::Client& client){ // Data handler
  ...
  client.sendData("server have data!\0", sizeof("server have data!\0"));
},
...
int main() {
  ...
  try {
    ...
    if(server.start() == TcpServer::status::up) {
      std::cout<<"Server started listen on port: " << server.getPort() << std::endl
               <<"Server handling thread pool size of: " << server.getThreadPool().getThreadCount() << std::endl;
      server.joinLoop();
      return EXIT_SUCCESS;
    } else {
